...
export default function RejectedCompanies() {
  ...
  const fetch = () => {
    setLoading(true)
    adminApi.getRejectedCompanies().then(res => setCompanies(res.data)).catch(() => {}).finally(() => setLoading(false))
  }
...
function EmptyState() {
  return (
    <div className="rounded-xl border border-dashed border-[var(--card-border)] p-12 text-center">
      <p className="text-lg font-medium text-[var(--text-primary)]">No rejected applications</p>
      <p className="text-sm text-[var(--text-secondary)] mt-1">No companies have been rejected.</p>
    </div>
  )
}
