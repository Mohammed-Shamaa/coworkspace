...
export default function WorkspacesPage() {
  ...
  const openDetail = async (id: number) => {
    ...
    try {
      ...
      setError('Failed to load workspace detail')
    } finally {
    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20 text-[var(--text-secondary)]">
          ...
          <p className="text-lg font-medium">{search ? 'No matching workspaces' : 'No approved workspaces'}</p>
        </div>
      ) : (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-[var(--text-secondary)] mt-0.5">Workspace: <span className="text-[var(--text-primary)] font-medium">{ws.companyName}</span></p>
                  <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-xs text-[var(--text-secondary)]">
                    <span className="flex items-center gap-1"><Calendar className="w-3 h-3" /> Registered: {new Date(ws.createdAt).toLocaleDateString()}</span>
                    <span className="flex items-center gap-1"><Users className="w-3 h-3" /> {ws.memberCount ?? 0} members</span>
                    {ws.totalDesks != null && <span className="flex items-center gap-1"><Building2 className="w-3 h-3" /> {ws.totalDesks}/{ws.maxCapacity ?? '?'} capacity</span>}
                    {ws.hasMeetingRoom && <span className="flex items-center gap-1 text-green-600 dark:text-green-400"><CheckCircle className="w-3 h-3" /> Meeting Room</span>}
                    {ws.trialStartDate && trialDays != null && (
                      <span className={`flex items-center gap-1 ${trialDays <= 0 ? 'text-red-500' : 'text-amber-500'}`}>
                        <Clock className="w-3 h-3" />
                        {trialDays <= 0 ? 'Trial Ended' : `${trialDays} trial days left`}
                      </span>
                    )}
                    {ws.subscriptionExpiryDate && (
                      <span className={`flex items-center gap-1 ${isExpired(ws.subscriptionExpiryDate) ? 'text-red-500' : ''}`}>
                        <CreditCard className="w-3 h-3" />
                        {isExpired(ws.subscriptionExpiryDate) ? 'Expired' : `Expires ${new Date(ws.subscriptionExpiryDate).toLocaleDateString()}`}
                      </span>
                    )}
                  </div>
                </div>
                <span className={`text-xs font-medium px-2 py-1 rounded-full shrink-0 ${
                  ...
                  {ws.paymentStatus === 'Active' ? 'Paid' : ws.paymentStatus}
                </span>
              </div>
            </div>
            )})}
          ...
          <div className="fixed top-0 right-0 h-full w-full max-w-lg bg-[var(--card-bg)] border-l border-[var(--card-border)] shadow-2xl z-50 overflow-y-auto">
            <div className="p-5">
              <div className="flex items-center justify-between mb-5">
                ...
                <h2 className="font-bold text-lg text-[var(--text-primary)]">Workspace Detail</h2>
                <div className="w-9" />
              </div>
              ...
              {detailLoading ? (
                ...
                <div className="space-y-5">
                  ...
                  <div className="bg-[var(--hover-bg)] rounded-xl p-4">
                    ...
                    <div className="flex gap-2 mt-2">
                      <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                        ...
                        {detail.paymentStatus === 'Active' ? 'Paid' : detail.paymentStatus}
                      </span>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="bg-[var(--hover-bg)] rounded-lg p-3">
                      <div className="text-xs text-[var(--text-secondary)]">Meeting Reservations</div>
                      <div className="text-xl font-bold text-[var(--text-primary)]">{detail.meetingRoomReservationCount}</div>
                    </div>
                    <div className="bg-[var(--hover-bg)] rounded-lg p-3">
                      <div className="text-xs text-[var(--text-secondary)]">Total Desks</div>
                      <div className="text-xl font-bold text-[var(--text-primary)]">{detail.totalDesks ?? 'N/A'}</div>
                    </div>
                    <div className="bg-[var(--hover-bg)] rounded-lg p-3">
                      <div className="text-xs text-[var(--text-secondary)]">Max Capacity</div>
                  ...
                  <div className="bg-[var(--hover-bg)] rounded-xl p-4 space-y-2">
                    <h4 className="font-semibold text-sm text-[var(--text-primary)]">Subscription</h4>
                    <div className="space-y-1 text-sm text-[var(--text-secondary)]">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className={`text-[var(--text-primary)] ${(() => { const d = remainingTrialDays(detail.trialStartDate); return d != null && d <= 0 ? 'text-red-500 font-medium' : '' })()}`}>
                          {(() => { const d = remainingTrialDays(detail.trialStartDate); return d != null ? (d <= 0 ? 'Ended' : `${d} days`) : 'N/A' })()}
                        </span>
                      </div>
                      <div className="flex justify-between">
                  ...
                  {/* Payment controls */}
                  <div className="space-y-2">
                    <h4 className="font-semibold text-sm text-[var(--text-primary)]">Payment Controls</h4>
                  ...
                  {detail.recentMembers.length > 0 && (
                    <div>
                      <h4 className="font-semibold text-sm text-[var(--text-primary)] mb-2">Recent Members</h4>
                      <div className="space-y-2">
                  ...
                  {detail.openingTime && (
                    <div className="text-xs text-[var(--text-secondary)]">
                      Working Hours: {detail.openingTime} - {detail.closingTime}
                    </div>
                  )}
                </div>
              ) : (
                <div className="text-center py-20 text-[var(--text-secondary)]">Failed to load detail</div>
              )}
            </div>
          </div>
        </>
